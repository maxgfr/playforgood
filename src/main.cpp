...
#if defined(NDEBUG)
# error "Playforgood cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xd779b4820c667411cacfb04dbcd1c21717942b5eca578ba3d72cf67a3ca9e177");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Playforgood: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Playforgood Signed Message:\n";
...
{
    // Playforgood: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10); // Playforgood: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 0.007 * 24 * 60 * 60; // Playforgood: 3.5 days
static const int64 nTargetSpacing = 2 * 60; // Playforgood: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Playforgood: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Playforgood: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "13/06/2018 Atelier créer sa propre crypto-monnaie";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 10 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04b463da0225dcd006df7848f99c4616f96e2a3863085474673bfc1ee617e757ffee41872b03f7f83ecb191b7bcc12333c5a6371bc937d1e8bd230b9f8d43fb7be") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1528915303;
        ...
        assert(block.hashMerkleRoot == uint256("0x828e621d56e6cc2aeb5abb066ee73ee1b5f9bbb0f48235675ee9dc688d3c3b6d"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc2, 0xb1, 0xdb }; // Playforgood: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("PlayforgoodMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("PlayforgoodMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static PlayforgoodMiner(CWallet *pwallet)
{
    printf("PlayforgoodMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("playforgood-miner");
    ...
    try { loop {
        ...
        printf("Running PlayforgoodMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("PlayforgoodMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&PlayforgoodMiner, pwallet));
}
